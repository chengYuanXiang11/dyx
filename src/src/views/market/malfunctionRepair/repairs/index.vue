<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="tableData" class="yg-table">
      ...
      <el-table-column label="下单时间" prop="create_datetime" />
      <el-table-column label="预约时间" prop="pre_time" />
      <el-table-column label="商户名称" prop="shop_manage" />
      <el-table-column label="预计上门时间" prop="pre_order_time" v-if="query.repair_status != 1"/>
      <el-table-column
        label="完成时间"
        prop="finish_time"
      ...
      >
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="text" @click="detailsTo(scope.row.id,1)"
            >查看</el-button
          >
          <el-button size="mini" type="text" @click="detailsTo(scope.row.id,2)" v-if="query.repair_status == 1"
            >完成</el-button
          >
        </template>
      </el-table-column>
