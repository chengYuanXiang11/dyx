<template>
  <div class="dashboard-editor-container app-container">
      ...
      <el-form-item label="月份选择">
        <el-col :span="6" class="month">
          <el-date-picker
            ...
            style="width: 100%"
            :validate-event="true"
            @change="(newV,oldV)=>handlerShow(getMonth,newV)"
            :picker-options="pickerOptions"
          >
          </el-date-picker> </el-col
        ><el-col :span="6" :offset="1" >
          <el-input v-model="employeeName" placeholder="可输入员工姓名搜索" suffix-icon="el-icon-search">
          </el-input>
        </el-col>
      </el-form-item>
    </el-form>
    ...
    <!-- 保存 恢复 导入 导出 -->
   <el-col class="row-bg"   :style="{ width: `${tableWidth}px` }">
    <el-col :span="6" :offset="2" class="checkboxLeft">
        <el-checkbox v-model="choice_shifit">未排版时，员工可自行选择班次打卡</el-checkbox >
    </el-col>
    <el-col  v-show="showButton <= 0" class="excel-col">
        <span class="execl-font">
            Excel排班 :
        </span>
        <el-button type="primary" class="row-button row-button1">第一步:导出排班表</el-button>
         <svg-icon icon-class="Right-icon" :style="{margin:'0px 5px'}"></svg-icon>
            <el-button type="primary">第二步:导入排班表</el-button>
    </el-col>
         <el-col  v-show="showButton > 0" class="submit-col">
        已调整：<span>{{ showButton }}</span>项
        ...
        >
          <template slot="header">
            <div
              ...
              :style="{'background': (new Date().getDay()===o.day&&date1.getYear() ==new Date().getYear()&&date1.getMonth() ==new Date().getMonth())?'#1585ff':''}"
            >
              {{ o.day }}<br />
              {{ o.week }}
            </div>
          ...
          <template slot-scope="scope">
               <el-tooltip class="item" effect="dark" v-if="isShowTooltip(index, scope.row)"  :open-delay="700" placement="top-start" :disabled="false">
                   <template slot="content" >
                   <!-- 对应tooltip悬浮显示 -->
                        <div v-html="formatTooltip(index, scope.row)" >
                        </div>
                    </template>
                        <div
                    class="table-row"
                    @click="changeShift($event, index, 'row', scope.$index)"
      ...
      >
        ...
        <div class="popover-main">
          <el-radio-group v-model="schedulingType">
            ...
            <el-radio-button
              label="2"
              :disabled="popoverOption.type == 'header'||scheduling.ruleForm.cycle.length<=0"
          ...
          <div class="scheduling-list" v-show="schedulingType == '1'">
            ...
            <ul>
              <li
                v-for="(o, index) in scheduling.existingShift.data"
                :key="o.id"
                class="title-td"
          ...
          <div class="scheduling-list" v-show="schedulingType == '2'">
            <div class="title">从该天开始，按周期排班，直至本月最后一天</div>
            <ul>
              <li @click="setShifts">{{scheduling.ruleForm.name}}</li>
            </ul>
          </div>
        </div>
      </div>
...
//  排班接口
import {getShift,addShift,getNameShift ,putShift} from '@/request/api/personnel/checking/scheduling'
//  班次接口
 ...
 created() {
  ...
  methods: {
            ...
            this.dataAll.forEach(value=>{
                ...
                if(v.year === value.year && v.month===value.month){
                        ...
                        value.userInfo.forEach((valueUse)=>{
                            let index = v.userInfo.findIndex(vUse=>(vUse.userId ===valueUse.userId &&vUse.userName ===valueUse.userName))
                            if(index ===-1){
    ...
    changeShift(event, index, type, rowIndex) {
      ...
      this.popoverTop = child.top + child.height + main.offsetTop - parent.top;
    ...
    format(index, val) {
      ...
      if (val.shift && val.shift[index]) {
        ...
        let existent=""
        // 当前下标 % 颜色数组长度，得到颜色
        const num =  this.scheduling.existingShift.data.findIndex((value) => {
              if (value.id === -1) {
                return false;
              }
        ...
        let nodeColor =
          this.scheduling.color[
            num% this.scheduling.color.length
          ];
            // 空白处理
            ...
            if(num === -1&& str!==""){
                 if (str.length <= 2) {
                        name = `<span  class="divColor ${existent}">${str}</span>`;
                    } else {
                        name = `<span  class="divColor ${existent}">
                        ${str.substring(0, 2)}
                        </br>
                        ${str.substring(2, 4)}
                        </span>`;
                     ...
                     if (str === '休息') {
                 name = `<span style="background:${nodeColor}"  class="divColor ${existent}">${str}</span>`;
                }
            }
    ...
    formatTooltip(index,val){
        let id = val.shift[index].id
         let ind =this.scheduling.existingShiftAll.findIndex(v=>v.id===id)
        if(ind!==-1 && id !== ""){
            let data =this.scheduling.existingShiftAll[ind]
              return `${data.working_shift_name}${data.first_working_time||''}${data.first_offduty_time||''}${data.second_working_time||''}${data.second_offduty_not_clock||''}${data.third_working_time||''}${data.third_offduty_time||''}`
        }else{
            if(id === -1){
                return `休息`
            }else{
   ...
   isShowTooltip(index,val){
    ...
    setShift(setType, o) {
      ...
      let val =
        setType == "clear"
          ? undefined
          : { working_shift_name: o.working_shift_name,name: o.working_shift_name,new: true ,date:index+1,id:o.id};
    ...
    setShifts() {
        ...
        for (let i = 0; i <  this.headers.length - index; i++) {
            ...
            arr=JSON.parse(JSON.stringify(this.scheduling.ruleForm.cycle[dataIndex]))
            arr.date = index+i+1;
            arr.new = true
            arr.name = arr.working_shift_name
            this.$set(
                this.tableData2[rowIndex].shift,
                index + i,
                arr
            );
            arr={}
    ...
    initializationTabular( products) {
            return new Promise((response, reject) => {
                let { tableData, refForm,date,type,employeeName,dayNum} = products;
                getNameShift(this.$route.query.id, date,employeeName)
                    .then((res) => {
                            ...
                            this.choice_shifit = data.choice_shift ==1?true:false;
                                // 排班数据id，用于更新排班
            ...
            return new Promise((response, reject) => {
                ...
                getshiftList({
                    ...
                    tag = res.data.results.filter((v) => {
                        return this.scheduling.existingShift.data.includes(v.id);
                    });
                    ...
                    this.$store.commit("scheduling/setExistingShiftAll", res.data.results);
    ...
    tagColor(item, index) {
      if (item.color) {
        ...
        return {
          background: this.scheduling.color[index % this.scheduling.color.length]
        };
      }
    },
    // 月份是否修改提示
      ...
      if (this.showButton > 0) {
        ...
        this.$confirm(null, {
          ...
          message: h("div", null, [
            h("i", {
              class:
                "el-message-box__status el-icon-warning    el-message-box__status_postation",
            }),
            h(
              "div",
