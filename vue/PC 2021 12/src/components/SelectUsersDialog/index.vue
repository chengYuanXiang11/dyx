<template>
    ...
    <div class="select-main">
        <div class="select-main-body">
            ...
            <el-button type="primary" @click="promoterVisible = true"
                >选择</el-button
            >
        </div>
        <el-dialog
...
export default {
    ...
    methods: {
        ...
        reset() {
            if (this.type == "all" && !this.multiple) {
                throw new Error("type为all时，multiple必须为true");
            } else {
                if (this.type == "all") {
                    this.selectedUsers = clone(this.datas.users);
                    this.selectedDepts = clone(this.datas.depts);
                } else {
