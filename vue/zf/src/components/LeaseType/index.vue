<template>
  <div
    class="select-main"
    :class="{ asterisk: rules, label_error_bottom: verification }"
  >
    <div
      @click="selectUsers3"
      class="Shift"
      :class="{ label_error_border: rules && verification }"
    >
      <span v-for="item in tagsList" :key="item.id + item.type">
        <el-tag
          class="shifit_tag"
    ...
    <div class="label_error" v-show="rules && verification">{{ message }}</div>
...
export default {
  ...
  methods: {
    ...
    surePromoter() {
      ...
      this.tagsList = JSON.parse(JSON.stringify(this.selectedUsers));
      this.$emit("confirm", this.tagsList);
      this.$emit("input", this.tagsList);
    },
    // 接口获取数据
    getPerson(o) {
      // 加路径
