...
exports.install = function (Vue, options) {
     ...
     * @param {*} router 返回指定页
     */
    Vue.prototype.$closePage = (router,query) => {
        let view = vm.$route;
        store
            ...
            .then(({
                ...
                if (router) {
                    vm.$router.push({path:router,query});
                } else {
                    const latestView = visitedViews.slice(-1)[0];
                    if (latestView) {
                        vm.$router.push({path:latestView.fullPath,query});
                    } else {
                        vm.$router.push("/");
                    }
                }
    ...
    Vue.prototype.queryFormat = (data, config) => {
        let paging = filterObj(config, ['pageNum', 'pageSize']),queryParams = deepClone(data);
        for (var key in queryParams) {
            if (key == 'daterange' && queryParams[key] !== null && queryParams[key].length > 0) {
                queryParams = Object.assign(queryParams, {
                    as: JSON.stringify({
                        create_datetime__range: queryParams[key]
                    })
                })
                delete queryParams.daterange
            }
        }
        return Object.assign(paging, queryParams)
    };
    /**
     * 成功提示
     */
