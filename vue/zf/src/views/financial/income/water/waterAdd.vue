<template>
  <FormPage
    ...
    <el-form ref="form" :model="form" label-width="120px">
      <el-form-item label="名称" prop="WaterRateName" :rules="rules">
        <div class="inpWidth">
          <el-input v-model="form.WaterRateName" maxlength="16" show-word-limit placeholder="请输入名称"></el-input>
        </div>
      </el-form-item>
...
} from "@/request/api/financial/income/water/index";
export default {
  name: "waterAdd",
  data() {
    return {
  ...
  methods: {
            ...
            if (_this.$route.query.type == 'add') {
              addRow(_this.form).then(res => {
                if (res.code == '200') {
                  _this.loading = false
                  _this.$success("操作成功!")
                  _this.$closePage('/financial/income/water')
                }
              }).catch(_ => {
                _this.loading = false
                _this.$error("操作失败!")
    ...
    cancel() {
      this.$closePage('/financial/income/water')
    }
  }
}
</script>
