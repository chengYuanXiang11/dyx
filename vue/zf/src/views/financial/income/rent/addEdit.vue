  ...
  <FormPage @cancel="cancel" @submit="save" submitBtnText="确定" cancelBtnText="取消">
    ...
    >
      <el-form-item label="名称" prop="rentName" :rules="rulesBlur">
        <div class="inpWidth">
          <el-input v-model="form.rentName" maxlength="16" show-word-limit placeholder="请输入名称"></el-input>
        </div>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    clearChange(val, index) {
      ...
      // 支付金额周期按月  并且缴费方式切换为按固定方式设置为每月31天
      if (this.form.payment[index].paymentScheme.time == 'month' && this.form.payment[index].payType.type == 2) {
        this.dayList = 31
      }
    },
            ...
            if (_this.$route.query.type == 'add') {
              addRow(_this.form).then(res => {
                if (res.code == '200') {
                  _this.loading = false
                  _this.$success("操作成功!")
                  _this.$closePage('/financial/rent')
                }
              }).catch(_ => {
                _this.loading = false
                _this.$error("操作失败!")
