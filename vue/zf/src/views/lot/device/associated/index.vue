<template>
  ...
  >
    <template slot="action-bar">
      <el-radio-group v-model="search.lotLocation" @change="getList()">
    ...
    <Drawer
      ...
      :showFooter="drawerType == 'edit' ? true : false"
      @confirm="confirmForm"
      @close="drawerClosed"
      @cancel="drawerClosed"
    >
...
<script>
// 关联设备
import { getList, putList } from "@/request/api/lot/device/associated";
// 设备管理
import {
  getList as deviceList,
  getListAll,
...
export default {
  ...
  created() {
    // 获取区域 建筑 楼层的select数据
    Promise.all([getArea(), getBuilding(), getFloor()]).then((res) => {
      let [area, building, floor] = res;
      this.search.form[1].options = this.optionLabel(area.data.results);
      this.search.form[2].options = this.optionLabel(building.data.results);
      this.search.form[3].options = this.optionLabel(floor.data.results);
    });
    this.getList();
  },
  methods: {
    ...
    remoteMethod(query) {
      if (query !== "") {
        this.equipmentOptionsloading = true;
        deviceList({ device_name__icontains: query })
          .then((res) => {
          ...
          .finally(() => {
            this.equipmentOptionsloading = false;
          });
      } else {
        this.equipmentOptions = [];
      }
    ...
    operation(type, row) {
      ...
      // 设备类型请求后再打开
      Promise.all([
        getListAll({ pageNum:"all" }),
        deviceList({ pageNum:"all" }),
      ])
        .then((res) => {
