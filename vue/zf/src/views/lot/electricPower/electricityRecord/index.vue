<template>
  ...
  >
    ...
    <el-table v-loading="loading" :data="tableData" border height="100%">
      ...
      <el-table-column key="2" label="电表编码" prop="code" v-if="radio1 == 1" />
      <el-table-column key="3" label="电表状态" prop="state" v-if="radio1 == 1" />
      ...
      <el-table-column key="7" label="商户名称" :formatter="nameFormatter" v-if="radio1 == 2"></el-table-column>
      <el-table-column key="5" label="经营品种" v-if="radio1 == 2">
        <template slot-scope="scope">{{ saleCategoryFilter(scope.row.merchant.saleCategory) }}</template>
      </el-table-column>
      <el-table-column key="8" label="商户姓名" v-if="radio1 == 2">
...
export default {
  ...
  methods: {
    nameFormatter(row) {
      if (this.radio1 == 2 && row.merchant && row.merchant.storeName) {
        return row.merchant.storeName
      }
    },
