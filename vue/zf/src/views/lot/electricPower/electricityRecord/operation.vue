  ...
  <Page>
    ...
    <div v-if="isShow">
      <el-descriptions title="电表信息">
        <el-descriptions-item labelClassName="labelClassName" label="电表当前状态">正常</el-descriptions-item>
        <el-descriptions-item labelClassName="labelClassName" label="电表当前数值">352KW/h</el-descriptions-item>
    ...
    <div>
      <el-table v-loading="loading" :data="tableData" border stripe class="yg-table">
        <el-table-column key="1" label="时间" prop="time" />
        <el-table-column key="2" label="上期抄表" prop="pre_count" v-if="isShow" />
        <el-table-column key="3" label="本期抄表" prop="now_count" v-if="isShow" />
        <el-table-column key="4" label="实用度数" prop="meter_count" />
        <el-table-column key="5" label="应缴费用" prop="charge" />
        <el-table-column key="6" label="实缴费用" prop="act_charge" />
        <el-table-column key="7" label="扣费时间" prop="charge_time" v-if="isShow && activeIndex == 0" />
      </el-table>
    </div>
  </Page>
</template>
    ...
    getList() {
      ...
      getSearchList(this.queryFormat(this.queryParams, this.query))
        .then((response) => {
          ...
          // _this.query.total = response.data.count;

          if (((this.type == 'quotient') || (this.type == 'merchant' && this.activeIndexMore != 'all')) && response.data.length > 0) {
            console.log('图表');
            response.data.map(item => {
              _this.xData.push(item.time)
              _this.yData.push(item.act_charge)
            })
          ...
          /* if (response.data.length > 0 && this.activeIndexMore != 'all') {
            console.log('图表');
            response.data.map(item => {
              _this.xData.push(item.time)
              _this.yData.push(item.act_charge)
