<template>
    ...
    >
        ...
        <el-table v-loading="loading" :data="tableData" border height="100%">
            ...
            <el-table-column key="contractTime" label="签约时间" prop="contractTime" v-if="radio1 == 2">
                <template slot-scope="scope">
                    {{
                        scope.row.contractTime | datefilter
                    }}
                </template>
            </el-table-column>
            <el-table-column key="leaseStatus" label="状态" prop="leaseStatus">
                <template slot-scope="scope">
                    <el-tag :type="statusTagFormatter(scope.row.leaseStatus)">
                        {{
            ...
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
                <template slot-scope="scope">
                    ...
                    <el-button
                        size="mini"
                        type="text"
                        @click="edit(scope.row.id, 'merchantView')"
                        v-if="radio1 == 2"
                    >查看</el-button>
                    <el-button
                        size="mini"
                        type="text"
                        @click="edit(scope.row.id, 'merchantUpload')"
...
import { getList, getListTwo } from "@/request/api/market/merchantsMgmt/enter";
export default {
    data() {
        return {
            // 查询参数
    ...
    methods: {
        ...
        getList(val) {
            ...
            if (val == 1) {
                ...
                let searchDate = this.queryFormat(this.queryParams, this.search);
                if (searchDate.applyTime) {
                    searchDate.applyTime = searchDate.applyTime.join(",")
                }
                if (searchDate.contractTime) {
                    searchDate.contractTime = searchDate.contractTime.join(",")
                }
                getListTwo(searchDate)
                    .then((response) => {
                        _this.tableData = response.data.rows;
                        _this.search.total = response.data.total;
                    })
