...
export default {
    ...
    methods: {
        ...
        submit() {
            // console.log(this.formData.project_details);
						
						// console.log(obj)
            this.$refs["form"].validate((valid) => {
                if (valid) {
									let obj = this.formData.project_details
									for(let i=0;i<obj.length;i++){
										delete obj[i].options
										delete obj[i].optionsList
									}
									this.formData.project_details = obj
									let res = {
										state: "2", 
										project_details:this.formData.project_details
									}
									manageTask(res, this.id).then((resp) => {
										this.$toast("成功！");
										this.closePage();
									});
                } else {
