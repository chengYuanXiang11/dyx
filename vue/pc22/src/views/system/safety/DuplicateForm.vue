<template>
  ...
  >
    ...
    <el-form-item label="检查时长" label-position="left">
      <el-switch
        v-model="form.validate_duration.status"
        active-color="#048790"
        inactive-color="#ff4949"
        active-value="1"
        inactive-value="0"
      ...
      <el-form-item
        class="lableLeft"
        label-width="100px"
        prop="validate_duration"
        label="时长"
        v-if="form.validate_duration.status === '1'"
      >
        <el-input-number
          v-model="form.validate_duration.duration"
          :min="1"
          :step="1"
          step-strictly
          :controls="false"
...
export default {
  ...
  methods: {
    ...
    validateDurationChange() {
      this.$set(this.form.validate_duration, "duration", 1);
    },
  },
};
</script>
